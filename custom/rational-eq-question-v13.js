/**
 * Rational Equation — Learnosity Custom Question v13 (EXPERIMENTAL — local only)
 *
 * v13 additions over v10 (structural DN):
 *  - _extractDNFromMath is brace-depth aware: only TOP-LEVEL {{N}} dropdowns
 *    are extracted out of $...$ zones (v10 behavior preserved). {{N}} nested
 *    inside braces (\frac/\dfrac args, \sqrt radicand, ^{} exponents, _{}
 *    subscripts, log bases) is STRUCTURAL and stays in the math, flowing
 *    through the \htmlId marker pipeline like FM inputs.
 *  - _replaceMarkers detects a structural context (placeholder inside .katex)
 *    and passes a structural flag to _buildDropdown.
 *  - _buildDropdown(structural=true): compact trigger sized like an FM box
 *    (no widest-option sizing), placeholder glyph instead of "Select…",
 *    req-dd-structural class for CSS. Options list floats as before.
 *  - Structural DN options are expected to be LaTeX/math-only (short tokens);
 *    they render via _renderDNOption (KaTeX) in trigger and menu.
 *
 * Forked from v10 — every other byte identical. v10 stays the production
 * renderer; nothing references v13 unless explicitly opted in.
 *
 * v10 additions over v8:
 *  - SC (single choice) and MC (multiple choice) sub-step types
 *  - _buildChoiceGroup: renders radio/checkbox option grids with KaTeX
 *  - SC/MC validation: exact match, per-option feedback colors
 *  - SC/MC save/restore, teacher view with gray ticks on correct options
 *
 * v7 changes over v6:
...
LearnosityAmd.define(["jquery-v1.10.2"], function ($) {
    ...
    Question.prototype._buildDropdown = function (id, options, onChange, structural) {
        ...
        // (v13: skipped for structural dropdowns — they stay compact in math)
        if (!structural) requestAnimationFrame(function () {
    ...
    // ═══════════════════════════════════════════════════
    // v10: SC / MC CHOICE GROUP BUILDER
    // ═══════════════════════════════════════════════════

    Question.prototype._buildChoiceGroup = function (sec) {
        var self = this;
        var subtype = sec.subtype; // 'sc' or 'mc'
        var options = sec.options || [];
        var cols = sec.columns || 1;
        var shouldShuffle = sec.shuffle !== false; // default true

        // Build index array and shuffle a copy if needed
        var indices = [];
        for (var oi = 0; oi < options.length; oi++) indices.push(oi);
        if (shouldShuffle) {
        ...
        var elId = self.uid + "-choice-" + sec.id;
        var $group = $('<div class="req-choice-group cols-' + cols + '" id="' + elId + '"></div>');
        ...
        indices.forEach(function (origIdx) {
            ...
            if (self.isTeacher) {
                var isCorrect = typeof opt === 'object' && opt.correct;
                if (isCorrect) {
                    $option.append('<span class="req-teacher-tick">\u2713</span>');
                }
            }
            ...
            $option.on("click", function () {
                ...
                // Clear feedback on any selection change
                $group.find(".req-choice-option").removeClass("correct incorrect");
                var $fbPill = $("#" + self.uid + "-fbpill-" + sec.id);
                $fbPill.attr("class", "req-fb");
                var $hintBox = $("#" + self.uid + "-hint-" + sec.id);
                if ($hintBox.length) $hintBox.removeClass("visible");

                if (subtype === 'sc') {
                    // SC: deselect all, select this one
                    $group.find(".req-choice-option").removeClass("selected");
        ...
        // Attach getValue / setValue helpers on the DOM element
        $group[0].getValue = function () {
            var selected = [];
            $group.find(".req-choice-option.selected").each(function () {
                selected.push(parseInt(this.getAttribute("data-option-index")));
            });
            return selected;
        };

        $group[0].setValue = function (selectedIndices) {
            $group.find(".req-choice-option").removeClass("selected");
            if (!selectedIndices) return;
            selectedIndices.forEach(function (idx) {
                $group.find('.req-choice-option[data-option-index="' + idx + '"]').addClass("selected");
            });
        };

        $group[0].setDisabled = function (d) {
            $group.toggleClass("disabled", d);
        };
    ...
    Question.prototype._extractDNFromMath = function (tpl, inputs) {
        ...
        return tpl.replace(/(\$\$[\s\S]*?\$\$|\$[^$]*?\$)/g, function (mathBlock) {
            // v13: brace-depth-aware walk. Only TOP-LEVEL (depth 0) dropdown
            // placeholders are extracted to prose (v10 behavior). A {{N}}
    ...
    Question.prototype._replaceMarkers = function (root, prefix, inputs, mkId) {
        ...
        inputs.forEach(function (inp, idx) {
            ...
            if (inp && inp.type === "dropdown") {
                // v13: placeholder inside KaTeX output = structural position
                var structural = !!(phEl.closest && phEl.closest('.katex'));
                var $dd = self._buildDropdown(mkId(idx, "dd"), inp.options || [], function () { self._fireChanged(); }, structural);
                replacement = $dd[0];
            } else {
    ...
    Question.prototype.validateContainer = function (container, boxValues) {
        ...
        if (relMatch) {
            return this.validateContainerRelation(assembled, { answer: answer, method: container.method, constraints: container.constraints }, relMatch);
        }
    ...
    Question.prototype.buildEquationTableSection = function (sec) {
        ...
        if (sec.preamble) {
            ...
            $pre.html(self._formatTextBlock(self._sanitizeContent(sec.preamble)));
    ...
    Question.prototype.buildTextWithInputSection = function (sec) {
        ...
        var rawTpl = self._sanitizeContent(sec.template || "");
        ...
        // v10: SC/MC — append choice group after the prompt text
        var isChoice = sec.subtype === 'sc' || sec.subtype === 'mc';
        if (isChoice && sec.options && sec.options.length > 0) {
            var $choiceGroup = self._buildChoiceGroup(sec);
            $content.append($choiceGroup);
        }

        // Actions row — only if there are actual inputs to check
        var hasInputs = (sec.inputs && sec.inputs.length > 0) || isChoice;
        if (hasInputs) {
            ...
            // Comma hint — shown next to "Not quite right!" if any input expects comma-separated values (FM only, not DN)
        ...
        // Init MathQuill fields (skip for SC/MC — no MQ inputs)
    ...
    Question.prototype.buildExpression = function ($container, row, secId, rowIdx) {
        ...
        var rawContent = self._sanitizeContent(self._normalizeToContent(row));
    ...
    Question.prototype.unlockSection = function (idx) {
        ...
        // v10: SC/MC sub-steps have inputs:[] but are interactive (options array)
        var isChoice = sec.subtype === 'sc' || sec.subtype === 'mc';
        var isTWINoInputs = sec.type === "text-with-input" && (!sec.inputs || sec.inputs.length === 0) && !isChoice;
    ...
    Question.prototype.checkRowAnswer = function (sec, rowIdx) {
        ...
        if ($commaHintRow.length) $commaHintRow.toggle(!allCorrect);
        ...
        if (allCorrect) {
            var stepKeyOk = sec.id + "-" + rowIdx;
            delete self.attemptCounts[stepKeyOk];
            delete self.lockedSteps[stepKeyOk];
            delete self.retryMode[stepKeyOk];
    ...
    Question.prototype.checkSectionAnswer = function (sec) {
        ...
        // v10: SC/MC validation branch
        if (sec.subtype === 'sc' || sec.subtype === 'mc') {
            var choiceEl = document.getElementById(self.uid + "-choice-" + sec.id);
            if (!choiceEl || !choiceEl.getValue) return;
            var selected = choiceEl.getValue();
            var options = sec.options || [];

            // Build set of correct original indices
            var correctIndices = [];
            for (var ci = 0; ci < options.length; ci++) {
                var opt = options[ci];
                if (typeof opt === 'object' && opt.correct) correctIndices.push(ci);
            }

            if (sec.subtype === 'sc') {
                allCorrect = selected.length === 1 && correctIndices.length === 1 && selected[0] === correctIndices[0];
            } else {
                // MC: exact set match
                var selSorted = selected.slice().sort();
                var corSorted = correctIndices.slice().sort();
                allCorrect = selSorted.length === corSorted.length &&
                    selSorted.every(function (v, i) { return v === corSorted[i]; });
            }
            ...
            if (allCorrect) {
                ...
                var $btnSC = self._findSectionButton(sec.id);
                if (self.attemptCounts[sec.id] >= 2) {
                    self.lockedSteps[sec.id] = true;
                    $btnSC.text("Retry").prop("disabled", true).addClass("req-retry-locked");
                    $fbPill.attr("class", "req-fb wrong").text("Not quite right!");
                } else {
        ...
        if (allCorrect) {
            ...
            if (self.attemptCounts[sec.id] >= 2) {
                ...
                var $fbPillTWI = $("#" + self.uid + "-fbpill-" + sec.id);
                $fbPillTWI.attr("class", "req-fb wrong").text("Not quite right!");
            } else {
                self.retryMode[sec.id] = true;
                $btnTWI.text("Retry");
            }
    ...
    Question.prototype._lockSectionInputs = function (sec) {
        ...
        if (sec.subtype === 'sc' || sec.subtype === 'mc') {
            var choiceEl = document.getElementById(self.uid + "-choice-" + sec.id);
            if (choiceEl && choiceEl.setDisabled) choiceEl.setDisabled(true);
        } else if (sec.inputs) {
            sec.inputs.forEach(function (inp, ii) {
                if (inp.type === "dropdown") {
                    var dd = document.getElementById(self.uid + "-dd-" + sec.id + "-" + ii);
    ...
    Question.prototype._unlockSectionInputs = function (sec) {
        ...
        if (sec.subtype === 'sc' || sec.subtype === 'mc') {
            var choiceEl = document.getElementById(self.uid + "-choice-" + sec.id);
            if (choiceEl && choiceEl.setDisabled) choiceEl.setDisabled(false);
        } else if (sec.inputs) {
            sec.inputs.forEach(function (inp, ii) {
                if (inp.type === "dropdown") {
                    var dd = document.getElementById(self.uid + "-dd-" + sec.id + "-" + ii);
    ...
    Question.prototype._clearSectionFeedback = function (secId) {
        ...
        if (sec && (sec.subtype === 'sc' || sec.subtype === 'mc')) {
            var choiceEl = document.getElementById(self.uid + "-choice-" + secId);
            if (choiceEl) $(choiceEl).find(".req-choice-option").removeClass("correct incorrect");
        } else if (sec && sec.inputs) {
            sec.inputs.forEach(function (inp, ii) {
                if (inp.type === "dropdown") {
    ...
    Question.prototype.collectAllInputValues = function () {
        ...
        sections.forEach(function (sec) {
            if (sec.type === "equation-table") {
                ...
                // v10: SC/MC — store selected indices
                if (sec.subtype === 'sc' || sec.subtype === 'mc') {
                    var choiceEl = document.getElementById(self.uid + "-choice-" + sec.id);
                    var selected = choiceEl && choiceEl.getValue ? choiceEl.getValue() : [];
                    inputs[sec.id + "-choice"] = { selected: selected, correct: secCompleted };
                    return; // no FM/DN inputs for choice sub-steps
                }
    ...
    Question.prototype.populateFieldsFromSaved = function (savedInputs) {
        ...
        for (var key in savedInputs) {
            ...
            if (saved.selected !== undefined) {
                // v10: SC/MC choice group
                // key format: "secId-choice" — extract secId
                var secId = key.replace(/-choice$/, "");
                var choiceEl = document.getElementById(self.uid + "-choice-" + secId);
        ...
        for (var si = 0; si < sections.length; si++) {
            ...
            if (sec.type === "text-with-input") {
                if (self.completedSections[sec.id]) {
                    ...
                    $("#" + self.uid + "-actions-" + sec.id).hide();
                    $("#" + self.uid + "-tick-" + sec.id).css("visibility", "visible");
                    // v10: SC/MC — disable choice group
                    if (sec.subtype === 'sc' || sec.subtype === 'mc') {
                        var choiceEl = document.getElementById(self.uid + "-choice-" + sec.id);
                        if (choiceEl && choiceEl.setDisabled) choiceEl.setDisabled(true);
                    } else {
