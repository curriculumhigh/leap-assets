/**
 * Rational Equation — Learnosity Custom Question v12
 *
 * v12 additions over v10:
 *  - LEAP run-mode detection (practice / homework / in_class), read from the
 *    worksheet's React context (nodeType / inClass) by walking the fiber tree.
 *    Since the widget renders inline in the worksheet document, the same props
 *    LEAP's own items use are reachable here.
 *  - Practice mode: wrong answers never hard-lock — the Check/Retry pair stays
 *    enabled for unlimited attempts (infinite retry). in_class (and any case
 *    where the context can't be read) keep today's tutor-lock behavior intact.
 *
    ...
    Question._readLeapContext = function () {
        ...
        try {
            var keyStarting = function (el, prefix) {
                for (var k in el) { if (k.indexOf(prefix) === 0) return k; }
                return null;
            };
            var roots = [], els = document.querySelectorAll("body, body *"), i, el, ck, fk, r, f;
            for (i = 0; i < els.length; i++) {
                el = els[i];
                ck = keyStarting(el, "__reactContainer$");
                if (ck) { r = el[ck]; roots.push(r.current || r); continue; }
                fk = keyStarting(el, "__reactFiber$") || keyStarting(el, "__reactInternalInstance$");
                if (fk) { f = el[fk]; while (f && f.return) f = f.return; roots.push(f); break; }
            }
            var seen = new Set(), q = roots.slice(), n = 0, fib, p;
    ...
    Question.prototype.checkSectionAnswer = function (sec) {
        ...
        if (sec.subtype === 'sc' || sec.subtype === 'mc') {
            ...
            if (allCorrect) {
                ...
                if (self.attemptCounts[sec.id] >= 2 && !self._infiniteRetry()) {
        ...
        if (allCorrect) {
            ...
            if (self.attemptCounts[sec.id] >= 2 && !self._infiniteRetry()) {
                ...
                $fbPillTWI.attr("class", "req-fb").text("");
            } else {
                self.retryMode[sec.id] = true;
                $btnTWI.text("Retry");
            }
                ...
                for (var ri = 0; ri < sec.rows.length; ri++) {
                    ...
                    if (self.completedRows[sec.id][ri]) {
                        ...
                        if (self.lockedSteps[rowStepKey]) {
                            ...
                            $("#" + self.uid + "-fbpill-" + sec.id + "-" + ri).attr("class", "req-fb").text("");
                        }
            ...
            if (sec.type === "text-with-input") {
                if (self.completedSections[sec.id]) {
                    ...
                    if (self.lockedSteps[sec.id]) {
                        ...
                        $lockedBtn.text("Retry").prop("disabled", true).addClass("req-retry-locked");
                        $("#" + self.uid + "-fbpill-" + sec.id).attr("class", "req-fb").text("");
                    }
                    self.events.trigger("changed", self.getResponse());
                    return;
                }
