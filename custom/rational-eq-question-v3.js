/**
 * Rational Equation — Learnosity Custom Question v3
 *
 * v3 additions over v1:
 *  - Rich response format: includes all input values, completed state
...
LearnosityAmd.define(["jquery-v1.10.2"], function ($) {
    ...
    function loadScript(url, check) {
        ...
        return $.getScript(url).fail(function (jqxhr, settings, exception) {
            console.error("[rational-eq-v3] Failed to load: " + url, exception);
        });
    }
    ...
    function Question(init, lrnUtils) {
        ...
        loadDeps().then(function () {
            ...
            console.error("[rational-eq-v3] loadDeps failed:", err);
