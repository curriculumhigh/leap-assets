/**
 * Rational Equation — Learnosity Custom Question v6
 *
 * v6 changes over v5:
 *  - Unified rendering: single buildExpression() replaces both
 *    buildTemplateExpression() and buildMixedContentRow()
 *  - All rows normalized to content format at render time
 *  - Principle: never put interactive inputs inside KaTeX
 *  - Fractions, superscripts, subscripts with {{N}} are split out as
 *    HTML sibling elements, not KaTeX children
 *
...
LearnosityAmd.define(["jquery-v1.10.2"], function ($) {
    ...
    Question.prototype.buildExpression = function ($container, row, secId, rowIdx) {
        ...
        var prefix = "REQV6" + secId.replace(/[^a-zA-Z0-9]/g, "") + "R" + rowIdx + "X";
        ...
        // ── Step 3: Replace non-fraction/sup/sub placeholder markers with MQ/dropdown inputs ──
        self._replaceMarkers($container[0], prefix, row.inputs, mkId);

        // ── Step 4: Init MathQuill fields ──
        requestAnimationFrame(function () {
