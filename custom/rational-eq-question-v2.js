    ...
    Question.prototype.checkRowAnswer = function (sec, rowIdx) {
        ...
        if (row.validation === "equivEquation" && row.inputs.length === 2) {
            ...
            [0, 1].forEach(function (ii) {
                ...
                if (slot) {
                    $(slot).removeClass("correct incorrect").addClass(allCorrect ? "correct" : "incorrect");
                    // Remove any existing validation icon, then add new one
                    $(slot).next('.req-valid-icon').remove();
                    $(slot).after('<span class="req-valid-icon ' + (allCorrect ? 'correct' : 'incorrect') + '">' + (allCorrect ? '✓' : '✗') + '</span>');
                }
            });
        } else {
            row.inputs.forEach(function (inp, ii) {
                ...
                var slot = document.getElementById(self.uid + "-mq-" + sec.id + "-" + rowIdx + "-" + ii);
                if (slot) {
                    $(slot).removeClass("correct incorrect").addClass(correct ? "correct" : "incorrect");
                    // Remove any existing validation icon, then add new one
                    $(slot).next('.req-valid-icon').remove();
                    $(slot).after('<span class="req-valid-icon ' + (correct ? 'correct' : 'incorrect') + '">' + (correct ? '✓' : '✗') + '</span>');
                }
        ...
        // (Inline validation icons are added directly after each input above)
    ...
    Question.prototype.checkSectionAnswer = function (sec) {
        ...
        sec.inputs.forEach(function (inp, ii) {
            if (inp.type === "dropdown") {
                ...
                // Remove any existing validation icon, then add new one
                $(select).next('.req-valid-icon').remove();
                $(select).after('<span class="req-valid-icon ' + (correct ? 'correct' : 'incorrect') + '">' + (correct ? '✓' : '✗') + '</span>');
                if (!correct) allCorrect = false;
            } else {
                ...
                if (slot) {
                    $(slot).removeClass("correct incorrect").addClass(correct2 ? "correct" : "incorrect");
                    // Remove any existing validation icon, then add new one
                    $(slot).next('.req-valid-icon').remove();
                    $(slot).after('<span class="req-valid-icon ' + (correct2 ? 'correct' : 'incorrect') + '">' + (correct2 ? '✓' : '✗') + '</span>');
                }
        ...
        if (allCorrect) {
            ...
            // Hide actions (inline icons already shown on each input)
