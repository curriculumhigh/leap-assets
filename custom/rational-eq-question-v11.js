/**
 * Rational Equation — Learnosity Custom Question v11
 *
 * v11 additions over v10:
 *  - Check/Retry flow: "Next" renamed to "Check"
 *  - Incorrect answers lock inputs and show "Retry" button
 *  - 2 consecutive failures lock the step (grayed "Retry") until teacher unlocks via allowRetry event
 *  - attemptCounts and lockedSteps persisted in getResponse/restoreFromResponse
 *
...
LearnosityAmd.define(["jquery-v1.10.2"], function ($) {
    ...
    // ── v11: Check/Retry helper methods ──
    ...
    Question.prototype._teacherAllowRetry = function (stepKey) {
        ...
        delete self.retryMode[stepKey];
        // Do NOT reset attemptCounts — next failure re-locks

        // Determine if this is an equation-table row or a TWI section
        var sec = self.findSectionById(stepKey);
        if (sec) {
            // TWI section (exact ID match)
            ...
            // Equation-table row: stepKey = "secId-rowIdx"
    ...
    Question.prototype.restoreFromResponse = function (response) {
        ...
        // Restore v11 attempt/lock state
            ...
            if (sec.type === "equation-table") {
                ...
                for (var ri = 0; ri < sec.rows.length; ri++) {
                    ...
                    if (self.completedRows[sec.id][ri]) {
                        ...
                        self.unlockedRows[sec.id][ri] = true;
                        // v11: If this step is locked, show disabled Retry button
